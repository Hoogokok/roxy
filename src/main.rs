...
use settings::{typestate::Validated, HttpsDisabled, HttpsEnabled};
use tracing::info;
use crate::{
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Settings 로드
    let settings_either: Either<Settings<Validated, HttpsDisabled>, Settings<Validated, HttpsEnabled>> = Settings::<Validated, HttpsDisabled>::load().await?;
